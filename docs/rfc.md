...
### 第二层：API key（运行时注入，不持久化）

`OPENCODE_AUTH_CONTENT` 通过 1Password `op run` 注入，只在容器环境变量里，不落盘。所有用户共享同一组 API key（运营者的 OpenAI key），因为 key 是运营者统一提供的。容器重启时重新从 1Password 解析。

`OPENCODE_AUTH_CONTENT` 的内容是 opencode auth JSON（`packages/opencode/src/auth/index.ts:59`）：
...
```json
{"openai":{"type":"api","key":"sk-..."},"ollama-cloud":{"type":"api","key":"..."}}
```
...
认证注入：`OPENCODE_AUTH_CONTENT` 环境变量。OpenCode 内置 basic auth 关闭（不设 `OPENCODE_SERVER_PASSWORD`），SSH key 是唯一认证层。
...
1Password 里存两个 Secure Note：

1. `opencode/auth_content`：opencode auth JSON（OpenAI key + 可选其他 provider key）
2. `tavily/api_key`：Tavily API key（纯字符串）
...
`op run` 在传给 docker compose 之前把 `op://...` 解析成真实值。docker compose 把它们作为环境变量传给所有 opencode 容器。
