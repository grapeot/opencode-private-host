...
1. 在 `keys/port_map` 里追加 `<username>:<port>`，再由 `scripts/render_compose.sh` 生成 `docker-compose.yml` 中的 `opencode-<username>` service
...
1. 从 `keys/port_map` 里移除对应用户，再由 `scripts/render_compose.sh` 从生成态 `docker-compose.yml` 里移除对应 service
...
- VPS 磁盘上：只有 `.env`（1Password 引用）、gitignored 的生成态 `docker-compose.yml` 和 Docker 运行状态
