...
2. 用户能通过 OpenCode web 界面连接自己的 ChatGPT subscription，后续对话正常。
3. 用户的 SSH key 只能 forward 到自己的 OpenCode 端口，执行 `ssh user@host ls` 等命令被拒绝。
4. 用户 A 无法访问用户 B 的 session、文件、容器。
5. API key 不出现在 VPS 磁盘的任何文件中（1Password 引用的 provider key 走环境变量注入；用户自己的 OAuth token 在各自 volume 里）。
6. 加一个用户的全流程：运营者执行一条 CLI 命令 + 用户提供公钥，完成。
7. 用户可以从多个设备访问（每个设备一把独立 key，通过 key management CLI 追加）。
8. OpenCode 容器重启不影响 SSH 网关（sshd-gateway 和各 opencode 容器独立）。
